...
function RemoveAcentos($Msg){
	//$palavra = ereg_replace("[^a-zA-Z0-9_]", "", strtr($Msg, "��������������������������", "aaaaeeiooouucAAAAEEIOOOUUC"));
	//return $palavra;
...
function sendMail($host,$username,$password,$de,$Reply,$Errors,$Priority,$para,$mensagem,$assunto,$anexos){
	...
	//
	if($username==''){
		$username = 'tecnetcg@gmail.com';
	}
	//
	if($de==''){
		$de = 'tecnetcg@gmail.com';
	}
	//
	if($de_nome==''){
		$de_nome = 'Provedor MikFácil';
	}
	//
	if($password==''){
		$password = 'amb142536';
	...
	// Envia o e-mail
	if (!$mail->send()) {
		$resp=array(false,'Mailer Error: ('.$mail->ErrorInfo.')');
	} else {
		$resp=array(true,'Email enviado com sucesso');
	}
	return $resp;
}
//
